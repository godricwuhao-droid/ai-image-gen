...
async def get_public_galleries(
    ...
    if style:
        base_query = base_query.where(Generation.prompt.ilike(f"%{style}%"))
    ...
    if style:
        count_query = count_query.where(Generation.prompt.ilike(f"%{style}%"))

    count_result = await db.execute(count_query)
    total = count_result.scalar()

    query = base_query.order_by(Generation.created_at.desc()).offset(offset).limit(page_size)
